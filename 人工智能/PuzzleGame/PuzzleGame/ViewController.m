...
- (IBAction)onSelectPictureButton:(UIButton *)sender {
    ...
    alert.popoverPresentationController.sourceRect = CGRectMake(sender.frame.size.width,0,0,0);
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"默认图片" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        weakSelf.image = [UIImage imageNamed:@"localimg"];
    }]];
...
- (IBAction)onHardButton:(UIButton *)sender {
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"超超高" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        weakSelf.matrixOrder = 7;
    }]];
    [alert addAction:[UIAlertAction actionWithTitle:@"超高" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        weakSelf.matrixOrder = 6;
    }]];
...
- (void)setMatrixOrder:(NSInteger)matrixOrder {
    if (matrixOrder < 3 || matrixOrder > 7) {
        return;
    }
    ...
    else if (matrixOrder == 6) {
        title = @"难度：超高";
    }
    else if (matrixOrder == 7) {
        title = @"难度：超超高";
    }
